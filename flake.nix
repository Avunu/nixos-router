            ...
            systemd.services = mkMerge [
              (mkIf cfg.cockpit.enable {
                cockpit = {
                  ...
                  # cockpit-tls needs to authenticate users.  In the normal
                  # architecture the wsinstance services pull it in, but since
                  # we run cockpit-tls directly here we must start it ourselves.
                  ...
                  serviceConfig = {
                    ...
                    ExecStart = [
                      ...
                      "${cfg.cockpit.package}/libexec/cockpit-tls --no-tls --port ${toString cfg.cockpit.port}"
                    ];
                    # # cockpit-tls ran as an ephemeral non-root DynamicUser; cockpit-tls
