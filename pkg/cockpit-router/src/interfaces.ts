...
export function validateNetwork(n: NetView): string[] {
  ...
  if (!n.wan.interface && n.wan.vlan == null) {
    errs.push("WAN: assign an untagged interface and/or set a VLAN id.");
  }
  if (lanIf.length === 0 && n.lan.vlan == null) {
    errs.push("LAN: assign interface(s) and/or set a VLAN id.");
  }
  ...
  if (owned.length === 0) {
    errs.push("Assign at least one physical interface to a network.");
  }
  const dup = uniq(owned.filter((p, i) => owned.indexOf(p) !== i));
  if (dup.length > 0) {
    errs.push(`Interface assigned to more than one network: ${dup.join(", ")}.`);
  }
  ...
  if (
    n.guest.enable &&
    n.guest.vlan != null &&
    tagged(n.guest.taggedInterfaces || []).length === 0
  ) {
    errs.push("Guest VLAN is set but no port carries its tag.");
  }

  if ((n.lan.taggedInterfaces || []).length > 0 && n.lan.vlan == null) {
    errs.push("LAN tagged interfaces are set but no VLAN id is set.");
  }
  if (n.guest.enable && (n.guest.taggedInterfaces || []).length > 0 && n.guest.vlan == null) {
  ...
  if (vids.length !== new Set(vids).size) {
    errs.push("VLAN ids must be distinct across networks.");
  }
  for (const v of vids) {
    if (v < 1 || v > 4094) {
      errs.push(`VLAN id ${v} is out of range (1–4094).`);
    }
  }
  ...
  const checkLen = (ports: string[], vid: number | null) => {
    ...
    for (const p of ports) {
      if (`${p}.${vid}`.length > 15) {
        errs.push(`VLAN sub-interface "${p}.${vid}" exceeds the 15-char kernel limit.`);
      }
    }
  };
  ...
  if (n.guest.enable) {
    checkLen(tagged(n.guest.taggedInterfaces || []), n.guest.vlan);
  }
