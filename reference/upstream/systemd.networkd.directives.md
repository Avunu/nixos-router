...
`systemd-networkd(8)`{.citerefentry}. See
`systemd.syntax(7)`{.citerefentry} for a general description of the
syntax.
...
`.network` files, and that it may be necessary to use
`systemd.link(5)`{.citerefentry}) or `systemd.netdev(5)`{.citerefentry})
files in conjunction with `.network` files when working with physical
and virtual network devices respectively.
...
collectively sorted and processed in alphanumeric order, regardless of
the directories in which they live. However, files with identical
filenames replace each other. It is recommended that each filename is
prefixed with a number smaller than `70` (e.g. `10-eth0.network`).
Otherwise, the default `.network` files or those generated by
`systemd-network-generator.service(8)`{.citerefentry} may take
...
`/run/` which in turn take precedence over those in `/usr/lib/`. Drop-in
files under any of these directories take precedence over the main
...
\[Network\] section specifying how the interface should be configured.
The first (in alphanumeric order) of the network files that matches a
...
specific network interfaces from `systemd-networkd(8)`{.citerefentry}\'s
management, while keeping the \[Match\] section generic. The
`ID_NET_MANAGED_BY=` property thus declares intended *ownership* of the
device, and permits ensuring that concurrent network management
...
:   Takes a boolean. If set to true, the IPv4 ARP (low-level Address
    ...
    this interface are enabled. For example, disabling ARP is useful
    when creating multiple MACVLAN or VLAN virtual interfaces atop a
    single lower-level physical interface, which will then only serve as
    a link/\"bridge\" device aggregating traffic to the same physical
    link and not participate in the network otherwise. Defaults to
    unset, and enabled when the IPv4 link-local addressing is enabled in
    `LinkLocalAddressing=` or an IPv4 address with
    `DuplicateAddressDetection=` enabled is requested. Otherwise, the
    kernel\'s default will be used.

`Multicast=`

:   Takes a boolean. If set to true, the multicast flag on the device is
    enabled. Defaults to unset and the flag is unchanged. When disabled,
    `IPv6AcceptRA=` and `IPv6SendRA=` cannot be enabled.
...
:   Takes a boolean, a minimum operational state (e.g., `carrier`), or a
    ...
    `degraded:routable`). Please see `networkctl(1)`{.citerefentry} for
    possible operational states. When `yes`, the network is deemed
    required when determining whether the system is online (including
    when running `systemd-networkd-wait-online`). When `no`, the network
    is ignored when determining the online state. When a minimum
    operational state and an optional maximum operational state are set,
    `systemd-networkd-wait-online` deems that the interface is online
    when the operational state is in the specified range.
    ...
    `systemd-networkd-wait-online.service(8)`{.citerefentry} for more
    details.
...
:   Takes an address family. When specified, an IP address in the given
    ...
    online (including when running `systemd-networkd-wait-online`).
    Takes one of `ipv4`, `ipv6`, `both`, or `any`. Defaults to `no`.
    Note that this option has no effect if `RequiredForOnline=no`.
...
:   Specifies the policy for `systemd-networkd` managing the link
    ...
    which is sometimes controlled by system administrators by running
    e.g., `ip link set dev eth0 up` or `ip link set dev eth0 down`, and
    can also be changed with `networkctl up eth0` or
    `networkctl down eth0`.

    Takes one of `up`, `always-up`, `manual`, `always-down`, `down`, or
    `bound`. When `manual`, `systemd-networkd` will not change the
    link\'s admin state automatically; the system administrator must
    bring the interface up or down manually, as desired. When `up` (the
    default) or `always-up`, or `down` or `always-down`,
    `systemd-networkd` will set the link up or down, respectively, when
    the interface is (re)configured. When `always-up` or `always-down`,
    `systemd-networkd` will set the link up or down, respectively, any
    time `systemd-networkd` detects a change in the administrative
    state. When `BindCarrier=` is also set, this is automatically set to
...
:   Enables DHCPv4 and/or DHCPv6 client support. Accepts `yes`, `no`,
    ...
    DHCP is not used for name resolution. See option `UseDomains=`
    below.
...
:   Takes a boolean. If set to `yes`, DHCPv4 server will be started.
    ...
    `PersistLeases=no/runtime` are specified in the \[DHCPServer\]
    section. It will be started after
    `systemd-networkd-persistent-storage.service` is started, which
    calls `networkctl persistent-storage yes`. See
    `networkctl(1)`{.citerefentry} for more details.

`LinkLocalAddressing=`

:   Enables link-local address autoconfiguration. Accepts a boolean,
    `ipv4`, and `ipv6`. An IPv6 link-local address is configured when
    `yes` or `ipv6`. An IPv4 link-local address is configured when `yes`
    or `ipv4` and when DHCPv4 autoconfiguration has been unsuccessful
    for some time. (IPv4 link-local address autoconfiguration will
    usually happen in parallel with repeated attempts to acquire a
    DHCPv4 lease).
    ...
    When IPv4 link-local addressing is enabled, `ARP=` is enabled unless
...
:   Takes a boolean or `resolve`. When true, enables [Link-Local
    Multicast Name Resolution](https://tools.ietf.org/html/rfc4795) on
    the link. When set to `resolve`, only resolution is enabled, but not
    host registration and announcement. Defaults to true. This setting
    is read by `systemd-resolved.service(8)`{.citerefentry}.

`MulticastDNS=`

:   Takes a boolean or `resolve`. When true, enables [Multicast
    DNS](https://tools.ietf.org/html/rfc6762) support on the link. When
    set to `resolve`, only resolution is enabled, but not host or
    service registration and announcement. Defaults to false. This
    setting is read by `systemd-resolved.service(8)`{.citerefentry}.

`DNSOverTLS=`

:   Takes a boolean or `opportunistic`. When true, enables
    [DNS-over-TLS](https://tools.ietf.org/html/rfc7858) support on the
    link. When set to `opportunistic`, compatibility with
    non-DNS-over-TLS servers is increased, by automatically turning off
    DNS-over-TLS servers in this case. This option defines a
    per-interface setting for `resolved.conf(5)`{.citerefentry}\'s
    global `DNSOverTLS=` option. Defaults to unset, and the global
    setting will be used. This setting is read by
    `systemd-resolved.service(8)`{.citerefentry}.

`DNSSEC=`

:   Takes a boolean or `allow-downgrade`. When true, enables
    [DNSSEC](https://tools.ietf.org/html/rfc4033) DNS validation support
    on the link. When set to `allow-downgrade`, compatibility with
    non-DNSSEC capable networks is increased, by automatically turning
    off DNSSEC in this case. This option defines a per-interface setting
    for `resolved.conf(5)`{.citerefentry}\'s global `DNSSEC=` option.
...
:   A space-separated list of DNSSEC negative trust anchor domains. If
    ...
    read by `systemd-resolved.service(8)`{.citerefentry}.
...
:   Controls support for Ethernet LLDP packet reception. LLDP is a
    ...
    Defaults to `routers-only`. Use `networkctl(1)`{.citerefentry} to
    query the collected neighbor data. LLDP is only available on
    Ethernet links. See `EmitLLDP=` below for enabling LLDP packet
    emission from the local system.
...
:   Controls support for Ethernet LLDP packet emission. Accepts a
    ...
    `machine-id(5)`{.citerefentry}) and the local interface name, as
    well as the pretty hostname of the system (as set in
    `machine-info(5)`{.citerefentry}). LLDP emission is only available
    on Ethernet links. Note that this setting passes data suitable for
    identification of host to the network and should thus not be enabled
    on untrusted networks, where such identification data should not be
    made available. Use this option to permit other systems to identify
    on which interfaces they are connected to this system. The three
    ...
    802.1AB-2016](https://standards.ieee.org/findstds/standard/802.1AB-2016.html).
    Note that configuring this setting to true is equivalent to
    `nearest-bridge`, the recommended and most restricted level of
    propagation. See `LLDP=` above for an option to enable LLDP
    reception.
...
:   A static IPv4 or IPv6 address and its prefix length, separated by a
    ...
    `inet_pton(3)`{.citerefentry}. This is a short-hand for an
    \[Address\] section only containing an Address key (see below). This
    ...
    Address Conflict Detection ([RFC
    5227](https://tools.ietf.org/html/rfc5227)) is enabled for the
    address. To assign an IPv4 link-local address without IPv4 Address
    Conflict Detection, please use \[Address\] section to configure the
    address and disable `DuplicateAddressDetection=`.
...
:   The gateway address, which must be in the format described in
    `inet_pton(3)`{.citerefentry}. This is a short-hand for a \[Route\]
    section only containing a `Gateway=` key. This option may be
    specified more than once.

    If an empty string is specified, then the all previous assignments
    in both \[Network\] and \[Route\] sections are cleared.

`DNS=`

:   A DNS server address, which must be in the format described in
    `inet_pton(3)`{.citerefentry}. This option may be specified more
    than once. Each address can optionally take a port number separated
    with `:`, a network interface name or index separated with `%`, and
    a Server Name Indication (SNI) separated with `#`. When IPv6 address
    is specified with a port number, then the address must be in the
...
:   Specifies the protocol-independent default value for the same
    ...
    below. Takes a boolean, or the special value `route`. See also the
    same setting in \[DHCPv4\] below. Defaults to unset.
...
:   A whitespace-separated list of domains which should be resolved
    ...
    This setting is read by
    `systemd-resolved.service(8)`{.citerefentry}. \"Search domains\"
    correspond to the `domain` and `search` entries in
    `resolv.conf(5)`{.citerefentry}. Domain name routing has no
    equivalent in the traditional glibc API, which has no concept of
...
:   An NTP server address (either an IP address, or a hostname). This
    ...
    `systemd-timesyncd.service(8)`{.citerefentry}.
...
:   Configures IPv4 packet forwarding for the interface. Takes a boolean
    ...
    option of the network interface. See [IP
    Sysctl](https://docs.kernel.org/networking/ip-sysctl.html) for more
    details about the sysctl option. Defaults to true if `IPMasquerade=`
    is enabled for IPv4, otherwise the value specified to the same
    setting in `networkd.conf(5)`{.citerefentry} will be used. If none
    of them are specified, the sysctl option will not be changed.

    To control the global setting, use the same setting in
    `networkd.conf(5)`{.citerefentry}.
...
:   Configures interface-specific host/router behaviour. Takes a boolean
    ...
    Sysctl](https://docs.kernel.org/networking/ip-sysctl.html) for more
    details about the sysctl option. Defaults to true if `IPMasquerade=`
    is enabled for IPv6 or `IPv6SendRA=` is enabled, otherwise the value
    specified to the same setting in `networkd.conf(5)`{.citerefentry}
    will be used. If none of them are specified, the sysctl option will
    not be changed.
    ...
    packets forwarded within the interfaces. This setting just controls
    the per-interface sysctl value, and the sysctl value is not directly
    correlated to whether packets are forwarded. To ensure IPv6 packets
    forwarded, the global setting in `networkd.conf(5)`{.citerefentry}
    needs to be enabled.

`IPMasquerade=`

:   Configures IP masquerading for the network interface. If enabled,
    packets forwarded from the network interface will appear as coming
    from the local host. Typically, this should be enabled on the
    ...
    and the global `networkd.conf(5)`{.citerefentry} unless they are
    explicitly specified. Similarly for `IPv6Forwarding=` when `ipv6` or
    `both` is specified. See `IPv4Forwarding=`/`IPv6Forwarding=` in the
    above for the per-link settings, and
    `networkd.conf(5)`{.citerefentry} for the global settings.

`IPv6PrivacyExtensions=`

:   Configures use of stateless temporary addresses that change over
    time (see [RFC 4941](https://tools.ietf.org/html/rfc4941), Privacy
    Extensions for Stateless Address Autoconfiguration in IPv6). Takes a
    boolean or the special values `prefer-public` and `kernel`. When
    true, enables the privacy extensions and prefers temporary addresses
    over public addresses. When `prefer-public`, enables the privacy
    extensions, but prefers public addresses over temporary addresses.
    When false, the privacy extensions remain disabled. When `kernel`,
    the kernel\'s default setting will be left in place. When
    unspecified, the value specified in the same setting in
    `networkd.conf(5)`{.citerefentry}, which defaults to `no`, will be
    used.
...
:   Takes a boolean. Controls IPv6 Router Advertisement (RA) reception
    ...
    This cannot be enabled on devices aggregated in a bond device, or
    when IPv6 link-local addressing (see `LinkLocalAddressing=`) or
    multicasting (see `Multicast=`) is disabled. Note, multicasting is
    disabled by default for some configurations, e.g. bridge ports.
    Hence, in that case `Multicast=` needs to be explicitly enabled to
    make this feature usable. When `IPv6SendRA=`, `IPv6Forwarding=`, or
    `IPMasquerade=` is enabled, this feature is disabled by default, but
    can be overridden by explicitly enabling this setting. Note,
    `IPv6Forwarding=` may be indirectly enabled when the global setting
    with the same name is enabled, or when `IPMasquerade=` is enabled on
    *any other interfaces*. See also `IPv6Forwarding=` and
    `IPMasquerade=` for more details. Enabled by default otherwise.

    Further settings for the IPv6 RA support may be configured in the
    \[IPv6AcceptRA\] section, see below.

    Also see [IP
    Sysctl](https://docs.kernel.org/networking/ip-sysctl.html) in the
    kernel documentation regarding `accept_ra`, but note that systemd\'s
...
:   Configures the maximum timeout for IPv4 Duplicate Address Detection
    (RFC 5227). Must be a value between 1 millisecond and 60 seconds. If
    set, Duplicate Address Detection takes a randomized time between 57%
    (4/7) and 100% of the given value. If unset, defaults to 200
    milliseconds.

`IPv4ReversePathFilter=`

:   Configure IPv4 Reverse Path Filtering. If enabled, when an IPv4
    packet is received, the machine will first check whether the
    *source* of the packet would be routed through the interface it came
    in. If there is no route to the source on that interface, the
    machine will drop the packet. Takes one of `no`, `strict`, or
    `loose`. When `no`, no source validation will be done. When
    `strict`, each incoming packet is tested against the FIB and if the
    incoming interface is not the best reverse path, the packet check
    will fail. By default, failed packets are discarded. When `loose`,
    each incoming packet\'s source address is tested against the FIB.
    The packet is dropped only if the source address is not reachable
    via any interface on that router. See [RFC
    3704](https://tools.ietf.org/html/rfc1027). When unset, the
    kernel\'s default will be used.

`MulticastIGMPVersion=`

:   Configures IPv4 Multicast IGMP Version to be used, and controls the
    value of `/proc/sys/net/ipv4/conf/INTERFACE/force_igmp_version`.
    Takes one of `no`, `v1`, `v2`, or `v3`. When `no`, no enforcement of
    an IGMP version is applied. IGMPv1/v2 fallbacks are allowed, and
    ...
    response is the same as with `no`. Defaults to unset --- the sysctl
    is not set.
...
:   Takes a boolean. Configures proxy ARP for IPv4. Proxy ARP is the
    ...
    destination. See [RFC 1027](https://tools.ietf.org/html/rfc1027).
...
:   Takes a boolean. Configures proxy ARP private VLAN for IPv4, also
    ...
    See [RFC 3069](https://tools.ietf.org/html/rfc3069). When unset, the
    kernel\'s default will be used.
...
:   Whether to enable or disable Router Advertisement sending on a link.
    ...
    \[IPv6SendRA\] section. If `DHCPPrefixDelegation=` is enabled, then
    the delegated prefixes are also distributed. See
...
:   Control whether Multi-protocol Label Switching (MPLS) routing is
    enabled on this interface. This configures
    `/proc/sys/net/mpls/conf/INTERFACE/input`. Takes a boolean. Defaults
    to unset, and the kernel\'s default will be used.

    Note, `systemd-networkd` does *not* load any required kernel modules
    for MPLS. To enable the feature, `mpls_router` kernel module must be
    loaded before `systemd-networkd.service` is started. Consider adding
    the kernel module to `modules-load.d(5)`{.citerefentry}.

`KeepMaster=`

:   Takes a boolean value. When enabled, the current master interface
    index will not be changed, and `BatmanAdvanced=`, `Bond=`,
    `Bridge=`, and `VRF=` settings are ignored. This may be useful when
    a netdev with a master interface is created by another program, e.g.
    `systemd-nspawn(1)`{.citerefentry}. Defaults to false.

`BatmanAdvanced=`; `Bond=`; `Bridge=`; `VRF=`

:   The name of the B.A.T.M.A.N. Advanced, bond, bridge, or VRF
    interface to add the link to. See
    `systemd.netdev(5)`{.citerefentry}.
...
:   The name of an IPoIB, IPVLAN, IPVTAP, MACsec, MACVLAN, MACVTAP,
    ...
    `systemd.netdev(5)`{.citerefentry}. This option may be specified
    more than once.
...
:   Takes a boolean. Allows `systemd-networkd` to configure a specific
    ...
    mechanisms like `DHCP=` and `IPv6AcceptRA=` (which is enabled by
    default in most cases) need to be disabled. Also,
    `DuplicateAddressDetection=` (which is enabled by default for IPv4
    link-local addresses and all IPv6 addresses) needs to be disabled
    for all static address configurations. Otherwise, without carrier,
    the interface will be stuck in the `configuring` state, and
    `systemd-networkd-wait-online` for the interface will timeout. Also,
    it is recommended to set `RequiredForOnline=no-carrier` to make
...
:   Takes a boolean or a timespan. When true, `systemd-networkd` retains
    ...
    - Enslaving a wireless interface to a bond interface, which may
      disconnect from the connected access point and causes its carrier
      to be lost.
    ...
    defaults to the value specified with `ConfigureWithoutCarrier=`.
...
:   Takes a boolean or one of `static`, `dynamic-on-stop`, and
    ...
    statically configured addresses and routes on starting up process.
    When `dynamic-on-stop`, the dynamically configured addresses and
    routes, such as DHCPv4, DHCPv6, SLAAC, and IPv4 link-local address,
    will not be dropped when `systemd-networkd` is being stopped. When
    `dynamic`, the dynamically configured addresses and routes will
    never be dropped, and the lifetime of DHCPv4 leases will be ignored.
    This is contrary to the DHCP specification, but may be the best
    choice if, e.g., the root filesystem relies on this connection. The
    setting `dynamic` implies `dynamic-on-stop`, and `yes` implies
    `dynamic` and `static`. Defaults to `dynamic-on-stop` when
    `systemd-networkd` is running in initrd, `yes` when the root
    filesystem is a network filesystem, and `no` otherwise.
...
:   Takes an IPv4 address or boolean value. The address must be in the
    format described in `inet_pton(3)`{.citerefentry}. If set to true,
    then the IPv4 broadcast address will be derived from the `Address=`
    setting. If set to false, then the broadcast address will not be
    set. Defaults to true, except for wireguard interfaces, where it
    defaults to false.
...
:   Takes a boolean. Designates this address the \"home address\" as
    defined in [RFC 6275](https://tools.ietf.org/html/rfc6275).
    Supported only on IPv6. Defaults to false.

`DuplicateAddressDetection=`

:   Takes one of `ipv4`, `ipv6`, `both`, or `none`. When `ipv4`,
    performs IPv4 Address Conflict Detection. See [RFC
    5227](https://tools.ietf.org/html/rfc5227). When `ipv6`, performs
    IPv6 Duplicate Address Detection. See [RFC
    4862](https://tools.ietf.org/html/rfc4862). Defaults to `ipv4` for
    IPv4 link-local addresses (169.254.0.0/16), `ipv6` for IPv6
    addresses, and `none` otherwise.

    When enabled, regardless implicitly or not, `ARP=` is enabled unless
...
:   Takes a boolean. If true the kernel manage temporary addresses
    ...
    [RFC 3041](https://tools.ietf.org/html/rfc3041). For this to become
    active, the use_tempaddr sysctl setting has to be set to a value
    greater than zero. The given address needs to have a prefix length
    of 64. This flag allows using privacy extensions in a manually
    configured network, just like if stateless auto-configuration was
...
:   Takes a boolean. Joining multicast group on ethernet level via
    `ip maddr` command would not work if we have an Ethernet switch that
...
:   This setting provides a method for integrating static and dynamic
    network configuration into Linux
    [NetLabel](https://docs.kernel.org/netlabel/index.html) subsystem
    rules, used by [Linux Security Modules
    (LSMs)](https://en.wikipedia.org/wiki/Linux_Security_Modules) for
    network access control. The label, with suitable LSM rules, can be
    used to control connectivity of (for example) a service with peers
    in the local network. At least with SELinux, only the ingress can be
    controlled but not egress. The benefit of using this setting is that
    it may be possible to apply interface independent part of NetLabel
    ...
    `netlabelctl(8)`{.citerefentry}, and the per-interface configuration
    with `systemd-networkd` once the interfaces appear later. Currently
    this feature is only implemented for SELinux.

    The option expects a single NetLabel label. The label must conform
    to lexicographic restrictions of LSM labels. When an interface is
    configured with IP addresses, the addresses and subnetwork masks
    will be appended to the [NetLabel Fallback Peer
    Labeling](https://github.com/SELinuxProject/selinux-notebook/blob/main/src/network_support.md)
    ...
    Once labeling is enabled for network traffic, a lot of LSM access
    control points in Linux networking stack go from dormant to active.
    Care should be taken to avoid getting into a situation where for
    example remote connectivity is broken, when the security policy has
    not been updated to consider LSM per-packet access controls and no
    rules would allow any network traffic. Also note that additional
    configuration with `netlabelctl(8)`{.citerefentry} is needed.
    ::::
...
:   This setting provides a method for integrating network configuration
    into firewall rules with
    [NFT](https://netfilter.org/projects/nftables/index.html) sets. The
    benefit of using the setting is that static network configuration
    (or dynamically obtained network addresses, see similar directives
    in other sections) can be used in firewall rules with the
    indirection of NFT set types. For example, access could be granted
    for hosts in the local subnetwork only. Firewall rules using IP
    address of an interface are also instantly updated when the network
    ...
    to lexicographic restrictions of NFT table names. The type of the
    element used in the NFT filter must match the type implied by the
    directive (`address`, `prefix` or `ifindex`) and address type (IPv4
    or IPv6) as shown in the table below.
    ...
    deconfigured. `systemd-networkd` only inserts elements to (or
...
3484](https://tools.ietf.org/html/rfc3484). Precedence is managed by
...
:   This specifies the Type of Service (ToS) field of packets to match;
    ...
    Notification (ECN) (the last 2 bits). See [Type of
    Service](https://en.wikipedia.org/wiki/Type_of_service) and
    [Differentiated
    services](https://en.wikipedia.org/wiki/Differentiated_services) for
    more details.
...
:   Specifies the routing table identifier to look up if the rule
    ...
    and `local`, and names defined in `RouteTable=` in
    `networkd.conf(5)`{.citerefentry}, or a number between 1
    and 4294967295. Defaults to `main`. Ignored if `L3MasterDevice=` is
    true.
...
:   Takes a boolean. Specifies whether the rule is to direct lookups to
    ...
    (VRF)](https://docs.kernel.org/networking/vrf.html). Defaults to
    false.
...
:   Takes a special value `ipv4`, `ipv6`, or `both`. By default, the
    ...
    `From=`. If neither `To=` nor `From=` are specified, then defaults
    to `ipv4`.
...
settings. Specify several \[NextHop\] sections to configure several
hops.
...
:   The id of the next hop. Takes an integer in the range
    ...
    specified in `networkd.conf(5)`{.citerefentry}. Otherwise, if
    unspecified, an unused ID will be automatically picked.
...
:   Takes a whitespace separated list of nexthop IDs. Each ID must be in
    ...
    weight after a colon (`id:weight`). The weight must be in the range
    1...255. If the weight is not specified, then it is assumed that the
    weight is 1. This setting cannot be specified with `Gateway=`,
...
:   Takes the gateway address or the special values `_dhcp4` and
    ...
    provided by DHCPv4 or IPv6 RA is used. When`_dhcp4`, the acquired
    DHCPv4 address will be used as the preferred source address of the
    route, unless it is explicitly configured in `PreferredSource=`.
...
:   Specifies the route preference as defined in [RFC
    4191](https://tools.ietf.org/html/rfc4191) for Router Discovery
    messages. Which can be one of `low` the route has a lowest priority,
    `medium` the route has a default priority or `high` the route has a
    highest priority.
...
:   The scope of the IPv4 route, which can be `global`, `site`, `link`,
    ...
    - `host` means the route will not leave the local machine (used for
      internal addresses like 127.0.0.1).
...
:   The preferred source address of the route. Takes `no` or an address
    in the format described in `inet_pton(3)`{.citerefentry}. If
    `Gateway=_dhcp4` is specified, defaults to the acquired DHCPv4
    address. Otherwise, defaults to unset. The value `no` may be useful
    to configure a route with `Gateway=_dhcp4` without setting preferred
...
:   The table identifier for the route. Takes one of predefined names
    ...
    in `networkd.conf(5)`{.citerefentry}, or a number between 1
    and 4294967295. The table can be retrieved using
    `ip route show table num`. If unset and `Type=` is `local`,
    `broadcast`, `anycast`, or `nat`, then `local` is used. In other
...
:   Specifies the type for the route. Takes one of `unicast`, `local`,
    ...
    defined route are discarded and the ICMP message \"Host
    Unreachable\" is generated. If `prohibit`, packets to the defined
    route are discarded and the ICMP message \"Communication
    Administratively Prohibited\" is generated. If `throw`, route lookup
    in the current routing table will fail and the route selection
    process will return to Routing Policy Database (RPDB). Defaults to
    `unicast`.
...
:   The TCP initial congestion window is used during the start of a TCP
    ...
    this option. When unset, the kernel\'s default (typically 10) will
    be used.
...
:   Configures multipath route. Multipath routing is the technique of
    using multiple alternative paths through a network. Takes a gateway
    address and/or a network interface name or index (prefixed with
    `@`). At least one of these must be specified. Optionally, a weight
    in 1..256 can be specified, separated with whitespace. This setting
...
:   Specifies the nexthop id. Takes an unsigned integer in the range
    1...4294967295. If set, the corresponding \[NextHop\] section must
    be configured. Defaults to unset.
...
:   When true (the default), the machine\'s hostname (or the value
    ...
    lower-case characters and no spaces, and be formatted as a valid DNS
    domain name. A single-label hostname is sent as DHCP option 12 (Host
    Name, [RFC 2132](https://tools.ietf.org/html/rfc2132)); a
    multi-label hostname (FQDN) is sent instead as DHCP option 81
    (Client FQDN, [RFC 4702](https://tools.ietf.org/html/rfc4702)).
...
:   Use this value for the hostname which is sent to the DHCP server,
    ...
    spaces, and be formatted as a valid DNS domain name. Multi-label
    hostnames (FQDNs) are acceptable; see `SendHostname=` above for
    details.
...
:   When configured, the specified Manufacturer Usage Description (MUD)
    ...
    most one MUD URL associated with them. See [RFC
...
:   The DHCPv4 client identifier to use. Takes one of `mac` or `duid`.
    ...
    IAID and DUID, is used. IAID can be configured by `IAID=`. DUID can
    be configured by `DUIDType=` and `DUIDRawData=`. When `Anonymize=`
    is enabled, `mac` is unconditionally used and the specified value
    will be ignored. When unspecified, the value specified in the same
    setting in `networkd.conf(5)`{.citerefentry} will be used.
...
:   Override the global `DUIDType=` setting for this network. See
    `networkd.conf(5)`{.citerefentry} for a description of possible
    values.
...
:   Takes a boolean. The DHCPv4 client can obtain configuration
    ...
    client configuration. See [RFC
    4039](https://tools.ietf.org/html/rfc4039) for details. Defaults to
    true when `Anonymize=no` and neither `AllowList=` nor `DenyList=` is
    specified, and false otherwise.

`Anonymize=`

:   Takes a boolean. When true, the options sent to the DHCP server will
    follow the [RFC 7844](https://tools.ietf.org/html/rfc7844)
    (Anonymity Profiles for DHCP Clients) to minimize disclosure of
    identifying information. Defaults to false.

    This option should only be set to true when `MACAddressPolicy=` is
    set to `random` (see `systemd.link(5)`{.citerefentry}).

    When true, `ClientIdentifier=mac`, `RapidCommit=no`,
    `SendHostname=no`, `Use6RD=no`, `UseCaptivePortal=no`, `UseMTU=no`,
    `UseNTP=no`, `UseSIP=no`, and `UseTimezone=no` are implied and these
    settings in the .network file are silently ignored. Also,
    `Hostname=`, `MUDURL=`, `RequestAddress=`, `RequestOptions=`,
...
:   Send an arbitrary raw option in the DHCPv4 request. Takes a DHCP
    ...
    may be escaped using [C-style
    escapes](https://en.wikipedia.org/wiki/Escape_sequences_in_C#Table_of_escape_sequences).
...
:   The Linux socket option `SO_PRIORITY` applied to the raw IP socket
    ...
    `socket(7)`{.citerefentry}. Can be used in conjunction with \[VLAN\]
    section `EgressQOSMaps=` setting of .netdev file to set the 802.1Q
    VLAN ethernet tagged header priority, see
    `systemd.netdev(5)`{.citerefentry}.

`BOOTP=`

:   Takes a boolean. When enabled, the DHCPv4 client will be configured
    to communicate with BOOTP servers, rather than with DHCP servers.
    Defaults to off.
...
:   When true (the default), the captive portal advertised by the DHCP
    ...
    displayed in the `networkctl(1)`{.citerefentry} status output
    per-link.

`UseDNR=`

:   When true, designated resolvers advertised by the DHCP server will
    be used as encrypted DNS servers. See [RFC
    9463](https://datatracker.ietf.org/doc/html/rfc9463).
...
:   Takes a boolean, or the special value `route`. When true, the domain
    ...
    over this link, similarly to the effect of the `Domains=` setting.
    ...
    similarly to the effect of the `Domains=` setting when the argument
    is prefixed with `~`.
    ...
    `networkd.conf(5)`{.citerefentry} will be used. When it is
    unspecified, the value specified in the same setting in the
    \[Network\] section in `networkd.conf(5)`{.citerefentry} will be
    used. When none of them are specified, defaults to `no`.
...
:   When true (the default), the static routes will be requested from
    ...
    direct route), a `link` scope will be used. For anything else, scope
    defaults to `global`.
...
:   The table identifier for DHCP routes. Takes one of predefined names
    `default`, `main`, and `local`, and names defined in `RouteTable=`
    in `networkd.conf(5)`{.citerefentry}, or a number between
    1...4294967295.
...
:   When true, and the DHCP server provides a Router option, the default
    ...
    always ignored regardless of this setting. See [RFC
    3442](https://datatracker.ietf.org/doc/html/rfc3442).
...
:   When true, subnets of the received IPv6 prefix are assigned to
    downstream interfaces which enables `DHCPPrefixDelegation=`. See
    also `DHCPPrefixDelegation=` in the \[Network\] section, the
    \[DHCPPrefixDelegation\] section, and [RFC
    5969](https://tools.ietf.org/html/rfc5969). Defaults to false.
...
:   Takes `none`, or one of the reject types: `unreachable`, `prohibit`,
    ...
    will be configured. See [RFC
    7084](https://datatracker.ietf.org/doc/html/rfc7084). If `none` is
    specified, such route will not be configured. This may be useful
    when custom firewall rules that handle packets for unassigned
...
:   When true, the DHCPv4 configuration will be delayed by the timespan
    ...
    timespan. See [RFC 8925](https://tools.ietf.org/html/rfc8925).
    Defaults to false.
...
:   A boolean. When true, `systemd-networkd` performs IPv4 Duplicate
    ...
    obtain an IP address again. See [RFC
    5227](https://tools.ietf.org/html/rfc5227). Defaults to false.
...
:   This applies the NFT set for the network configuration received with
    ...
    configuration. See `NFTSet=` in \[Address\] section for more
    details. For `address` or `prefix` source types, the type of the
    element used in the NFT filter must be `ipv4_addr`.
...
:   Send an arbitrary vendor option in the DHCPv6 request. Takes an
    ...
    data string may be escaped using [C-style
    escapes](https://en.wikipedia.org/wiki/Escape_sequences_in_C#Table_of_escape_sequences).
    This setting can be specified multiple times. If an empty string is
    specified, then all options specified earlier are cleared. Defaults
    to unset.
...
:   A DHCPv6 client can use User Class option to identify the type or
    ...
    escapes](https://en.wikipedia.org/wiki/Escape_sequences_in_C#Table_of_escape_sequences).
    This setting can be specified multiple times. If an empty string is
    specified, then all options specified earlier are cleared. Takes a
    whitespace-separated list of strings. Note that currently `NUL`
...
:   Takes an IPv6 address with prefix length in the same format as the
    ...
    include a prefix hint in the DHCPv6 solicitation sent to the server.
    The prefix length must be in the range 1...128. Defaults to unset.
...
:   Takes a boolean. The DHCPv6 client can obtain configuration
    ...
    faster client configuration. See [RFC
    3315](https://tools.ietf.org/html/rfc3315#section-17.2.1) for
    details. Defaults to true, and the two-message exchange will be used
    if the server support it.
...
:   Use this value for the hostname which is sent to the DHCPv6 server,
    instead of machine\'s hostname. Note that the specified hostname
    must consist only of 7-bit ASCII lower-case characters and no spaces
...
:   When true (the default), the client will request the DHCPv6 server
    ...
    `DHCPPrefixDelegation=` setting in the \[Network\] section, settings
    in the \[DHCPPrefixDelegation\] section, and [RFC
    8415](https://www.rfc-editor.org/rfc/rfc8415.html#section-6.3).
...
:   This applies the NFT set for the network configuration received with
    ...
    configuration. See `NFTSet=` in \[Address\] section for more
    details. For `address` or `prefix` source types, the type of the
    element used in the NFT filter must be `ipv6_addr`.
...
:   Specifies the name or the index of the uplink interface, or one of
    ...
    itself is considered the uplink interface, and `WithoutRA=solicit`
    is implied if the setting is not explicitly specified. When `:auto`,
...
:   Configure a specific subnet ID on the interface from a (previously)
    received prefix delegation. You can either set \"auto\" (the
    default) or a specific subnet ID (as defined in [RFC
    4291](https://tools.ietf.org/html/rfc4291#section-2.5.4), section
...
:   Takes a boolean. When enabled, and `IPv6SendRA=` in \[Network\]
    section is enabled, the delegated prefixes are distributed through
    the IPv6 Router Advertisement. This setting will be ignored when the
    `DHCPPrefixDelegation=` setting is enabled on the upstream
...
:   Specifies an optional address generation mode for assigning an
    ...
    false, then this setting will be ignored. Defaults to unset, which
...
:   Specifies an optional address generation mode for the Stateless
    ...
    :   The algorithm specified in [RFC
        7217](https://tools.ietf.org/html/rfc7217) will be used to
        generate interface identifiers. This mode can optionally take an
        IPv6 address separated with a colon (`:`). If an IPv6 address is
        ...
        secret key. See `sd-id128(3)`{.citerefentry},
        `sd_id128_from_string(3)`{.citerefentry}, and
        `sd_id128_get_machine(3)`{.citerefentry}.
...
:   Takes a boolean, or the special value `route`. When true, the domain
    ...
    searching, similarly to the effect of the `Domains=` setting when
    the argument is prefixed with `~`. Defaults to false.
...
:   The table identifier for the routes received in the Router
    ...
    `networkd.conf(5)`{.citerefentry}, or a number between
    1...4294967295.
...
:   When true (the default), the captive portal received in the Router
    ...
    and displayed in the `networkctl(1)`{.citerefentry} status output
    per-link.
...
:   When true, the IPv6 PREF64 (or NAT64) prefixes received in the
    ...
    programs and displayed in the `networkctl(1)`{.citerefentry} status
    output per-link. See [RFC
    8781](https://tools.ietf.org/html/rfc8781). Defaults to false.
...
:   Takes a boolean, or the special value `always`. When true, the
    ...
    an RA is received, even if neither the `managed` nor the
...
:   Specifies the server address for the DHCP server. Takes an IPv4
    ...
    For more details about the automatic address selection, see
...
:   Configures the pool of addresses to hand out. The pool is a
    ...
    address. `PoolOffset=` takes the offset of the pool from the start
    of subnet, or zero to use the default value. `PoolSize=` takes the
...
:   `EmitDNS=` takes a boolean. Configures whether the DHCP leases
    handed out to clients shall contain DNS server information. Defaults
    to `yes`. The DNS servers to pass to clients may be configured with
...
:   The `EmitRouter=` setting takes a boolean value, and configures
    ...
    then the server address will be used for the router option. When the
...
:   Takes a boolean. Configures whether the DHCP leases handed out to
    clients shall contain domain name information (DHCP option 15).
    Defaults to `no`.

`Domain=`

:   Takes a domain name (such as `example.com`) to pass to DHCP clients.
    This configures the DNS default domain for DHCP clients. When set,
    DHCP clients will use this as their DNS search domain.

    When `EmitDomain=yes` is set but `Domain=` is not configured, the
    domain name will be automatically derived from the system\'s fully
    qualified hostname. For example, if the system\'s hostname is
    `host.example.com`, the domain `example.com` will be sent to
    clients. If the system\'s hostname does not contain a domain part
    (e.g., hostname is just `host`), no domain name will be sent to DHCP
    clients. When empty or unset, defaults to no domain name.
...
:   Takes an IPv4 address of the boot server used by e.g. PXE boot
    ...
    of the DHCP message header. See [RFC
    2131](https://www.rfc-editor.org/rfc/rfc2131.html) for more details.
    Defaults to unset.
...
:   Takes a name of the boot server used by e.g. PXE boot systems. When
    ...
    name\"). See [RFC 2132](https://www.rfc-editor.org/rfc/rfc2132.html)
    for more details. Defaults to unset.
...
:   Takes a timespan. Controls the [RFC
    8925](https://tools.ietf.org/html/rfc8925) IPv6-Only Preferred
...
:   Takes an IPv4 address, which must be in the format described in
    `inet_pton(3)`{.citerefentry}. Turns this DHCP server into a DHCP
    relay agent. See [RFC 1542](https://tools.ietf.org/html/rfc1542).
    The address is the address of DHCP server or another relay agent to
    forward DHCP messages to and from.
...
:   Takes a boolean. When true, the server supports [RFC
    4039](https://datatracker.ietf.org/doc/html/rfc4039). When a client
    sends a DHCPDISCOVER message with the Rapid Commit option to the
    server, then the server will reply with a DHCPACK message to the
    client, instead of DHCPOFFER. Defaults to true.

`PersistLeases=`

:   Takes a boolean or special value `runtime`. When `yes`, the DHCP
    server will load and save leases in the persistent storage. When
    `runtime`, the DHCP server will load and save leases in the runtime
    storage, hence bound leases will be lost when the runtime storage is
    cleared by e.g. by calling
    `systemctl clean systemd-networkd.service` or the system is
    rebooted. When `no`, the DHCP server will neither load nor save
    leases in the persistent storage, hence bound leases will be lost
    when the interface is reconfigured e.g. by `networkctl reconfigure`,
    or `systemd-networkd.service(8)`{.citerefentry} is restarted. Using
    `runtime` and `no` may cause address conflict on the network after
    the leases are lost. So, please take an extra care when disable this
    setting. When unspecified, the value specified in the same setting
    in `networkd.conf(5)`{.citerefentry}, which defaults to `yes`, will
    be used.

    When `RelayTarget=` is specified, this setting will be ignored and
    no leases will be saved, as there will be no bound lease on the
    server.

`LocalLeaseDomain=`

:   Takes a DNS domain name as argument. If specified,
    `systemd-networkd.service(8)`{.citerefentry} will integrate with
    `systemd-resolved.service(8)`{.citerefentry} and ensure that the
    hostnames associated with each handed out DHCP lease may be resolved
    to the IP addresses of the lease. The hostnames are suffixed with
    the specified domain name.

    Note that this purely about hostname resolution on the local system,
    i.e. from programs with access to the same `systemd-resolved`
    instances via D-Bus IPC, Varlink IPC, or the local DNS stub.
...
:   The hostname to send to the client in DHCP replies. This can be
    either a simple hostname (e.g., `mydevice`) or a fully qualified
    domain name (e.g., `mydevice.example.com`), following RFC 1123
    naming conventions. Each label can be up to 63 characters, with a
    total maximum length of 253 characters for FQDNs. When this option
    is set, the DHCP server will include the hostname in DHCP replies
    (both OFFER and ACK) to the client with the matched MAC address.

    The server automatically selects the appropriate DHCP option based
    on the hostname format: simple hostnames (single DNS label) are sent
    via option 12 (Host Name) per RFC 2132, while FQDNs with multiple
    labels are sent via option 81 (Client FQDN) per RFC 4702 using DNS
    wire format encoding. The configured hostname is sent
    unconditionally, any hostname requested by the client in its DHCP
    message is ignored.
...
are defined with one or more \[IPv6Prefix\] or \[IPv6RoutePrefix\]
sections.

`Managed=`; `OtherInformation=`

:   Takes a boolean. Controls whether a DHCPv6 server is used to acquire
    IPv6 addresses on the network link when `Managed=` is set to `true`
    or if only additional network information can be obtained via DHCPv6
...
:   Configures IPv6 router preference if `RouterLifetimeSec=` is
    non-zero. Valid values are `high`, `medium` and `low`, with `normal`
    and `default` added as synonyms for `medium` just to make
    configuration easier. See [RFC
    4191](https://tools.ietf.org/html/rfc4191) for details. Defaults to
    `medium`.
...
:   `DNS=` specifies a list of recursive DNS server IPv6 addresses that
    ...
    When `EmitDNS=` is false, no DNS server information is sent in
...
:   A list of DNS search domains distributed via Router Advertisement
    ...
    DNS search domain information is sent in Router Advertisement
    messages. `EmitDomains=` defaults to true.

`DNSLifetimeSec=`

:   Lifetime in seconds for the DNS server addresses listed in `DNS=`
    and search domains listed in `Domains=`. Defaults to 3600 seconds
    (one hour).
...
:   Takes a boolean. Specifies that IPv6 router advertisements indicate
    ...
    Agent option. Defaults to false. See [RFC
    6275](https://tools.ietf.org/html/rfc6275) for further details.
...
announced via Router Advertisements. See [RFC
4861](https://tools.ietf.org/html/rfc4861) for further details.
...
:   Specifies an optional address generation mode for assigning an
    ...
    the \[IPv6AcceptRA\] section. If `Assign=` is set to false, then
    this setting will be ignored. Defaults to unset, which means the
...
that are announced via Router Advertisements. See [RFC
4191](https://tools.ietf.org/html/rfc4191) for further details.
...
8781](https://tools.ietf.org/html/rfc8781) for further details.
...
:   The IPv6 PREF64 (or NAT64) prefix that is to be distributed to
    ...
    Use multiple \[IPv6PREF64Prefix\] sections to configure multiple
    IPv6 prefixes since prefix lifetime may differ from one prefix to
...
:   Sets the \"priority\" of sending packets on this interface. Each
    ...
    decide which link to use. Lower value means higher priority. It is
    an integer value between 0 to 63. `systemd-networkd` does not set
    any default, meaning the kernel default value of 32 is used.

`Locked=`; `MACAuthenticationBypass=`

:   Takes a boolean. Configures whether the port is \"locked\" and does
    not allow traffic forwarded until fully authenticated, e.g. via
    802.1x. When unset, the kernel\'s default will be used.

:   Takes a boolean. Configures whether a locked port has \"MAC
    Authentication Bypass\" enabled and creates newly learned fdb
    entries in a \"locked\" state. User space can authenticate these
    entries by clearing the locked flag. Requires Learning to be
    enabled. When unset, the kernel\'s default will be used.

`VLANTunnel=`

:   Takes a boolean. Configures whether vlan tunnel mode is to be
    enabled on this port. When unset, the kernel\'s default will be
    used.
...
:   Specifies where the address is associated with. Takes one of `use`,
    ...
    address is associated with a router. Note that it is valid if the
...
:   Specifies the time quanta, propagation segment, phase buffer segment
    ...
    `PhaseBufferSegment1=`, `PhaseBufferSegment2=`, and `SyncJumpWidth=`
    take number of time quantum specified in `TimeQuantaNSec=` and must
    be an unsigned integer in the range 0...4294967295. These settings
    except for `SyncJumpWidth=` will be ignored when `BitRate=` is
    specified.

`DataBitRate=`; `DataSamplePoint=`

:   The bitrate and sample point for the data phase, if CAN-FD is used.
    These settings are analogous to the `BitRate=` and `SamplePoint=`
    keys.
...
:   CAKE places packets from different flows into different queues, then
    ...
    :   Flows are defined by source-destination host pairs. Equivalent
        to the same option for `tc qdisc` command. See also
        `tc-cake(8)`{.citerefentry}.
    ...
    :   Flows are defined by the entire 5-tuple of source address,
        ...
        command. See also `tc-cake(8)`{.citerefentry}.
    ...
    :   Flows are defined by the 5-tuple (see `flows` in the above), and
        ...
        `tc qdisc` command. See also `tc-cake(8)`{.citerefentry}.

    `dual-dst-host`

    :   Flows are defined by the 5-tuple (see `flows` in the above), and
        fairness is applied first over destination addresses, then over
        individual flows. Equivalent to the `dual-dsthost` option for
        `tc qdisc` command. See also `tc-cake(8)`{.citerefentry}.
...
master or enslaved device. To make the settings in this section take an
effect, `VLANFiltering=` option has to be enabled on the bridge master,
see the \[Bridge\] section in `systemd.netdev(5)`{.citerefentry}, and
each enslaved device needs to define the relevant VLAN IDs in its own
\[BridgeVLAN\] section, which it has in common with the bridge master.
All assigned VLAN IDs on the interface that are not configured in the
.network file will be removed. If VLAN IDs on an interface need to be
managed by other tools, then the settings in this section cannot be used
in the matching .network file and `VLANFiltering=` needs to be disabled
on the bridge master.
...
:   The VLAN ID specified here will be used to untag frames on egress.
    Configuring `EgressUntagged=` implicates the use of `VLAN=` above
    and will enable the VLAN ID for ingress as well. This can be either
    a single ID or a range M-N. This setting can be specified multiple
    times. If an empty string is assigned, then the all previous
...
:   The port VLAN ID specified here is assigned to all untagged frames
    ...
    Configuring `PVID=` implicates the use of `VLAN=` setting in the
    above and will enable the VLAN ID for ingress as well. Defaults to
    unset, and will keep the assigned port VLAN ID if exists.

# \[MobileNetwork\] Section Options

This section configures the default setting of the ModemManager
integration. See [](https://modemmanager.org/docs/modemmanager/) for
more information about ModemManager.

Regardless of the \[MobileNetwork\] section settings consider using the
following for LTE modems (take into account that LTE modems do not
typically support LLDP because LLDP is a Layer 2 protocol for Ethernet
networks and an LTE modem connects to a cellular network, not a local
Ethernet LAN):
...
The following options are available in the \[MobileNetwork\] section:

`APN=`

:   An Access Point Name (APN) is the name of a gateway between a mobile
    network (GSM, GPRS, 3G, 4G and 5G) and another computer network.
    Required in 3GPP. Defaults to unset and no attempt to establish the
    connection is made.

`AllowedAuthenticationMechanisms=`

:   Authentication method to use. Specifies the white-space separated
    list of `none`, `pap`, `chap`, `mschap`, `mschapv2`, or `eap`
    methods. Optional in 3GPP. Defaults to unset and an automatically
    picked authentication method will be used.
...
:   Addressing type. Takes one of `ipv4`, `ipv6`, `both`, or `any`.
    Optional in 3GPP and CDMA. Defaults to unset and automatically
    selected.

`AllowRoaming=`

:   A boolean. When true, connection is allowed during roaming. When
    false, connection is not allowed during roaming. Optional in 3GPP.
    Defaults to `yes`.

`PIN=`

:   SIM-PIN unlock code. Defaults to unset.

`OperatorId=`

:   ETSI MCC-MNC of a network to force registration. Defaults to unset.

`RouteMetric=`

:   Set the routing metric for routes specified by the mobile network
    (including the prefix route added for the specified prefix). Takes
    an unsigned integer in the range 0...4294967295. When unset or set
    to 0, the kernel\'s default value will be used.
...
:::: example
::: title
IPv6 Prefix Delegation (DHCPv4 6RD)
:::
...
:::: example
::: title
Bridge port with VLAN forwarding
:::
...
:::: example
::: title
Connecting to a cellular network with ModemManager
:::
...
This connects a cellular modem to a broadband network matched with the
network interface `wwan0`, with APN name `internet`, allowed
authentication `none`, `pcap`, or `chap`, user name `user`, their
password `pass`, allows both IPv4 and IPv6, does not allow roaming, SIM
card pin unlock code `1111`, only allows connecting to operator with MCC
`25503`, and sets up a default gateway with route metric of 30.
::::

# See Also

`systemd(1)`{.citerefentry},
`systemd-networkd.service(8)`{.citerefentry},
`systemd.link(5)`{.citerefentry}, `systemd.netdev(5)`{.citerefentry},
`systemd-network-generator.service(8)`{.citerefentry},
`systemd-resolved.service(8)`{.citerefentry}
