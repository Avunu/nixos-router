...
- **Handshake, but nothing on the LAN answers.** The device's `Address` must equal its entry in the router's **Allowed IPs**, and its `AllowedIPs` must include the LAN.
