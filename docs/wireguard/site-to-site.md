...
- **Forwarding is off on the tunnel.** The router turns on IPv4 forwarding for each tunnel. `sysctl net.ipv4.conf.wg0.forwarding` should print `net.ipv4.conf.wg0.forwarding = 1`. If it prints `0`, nothing from the tunnel is forwarded to the LAN; `sudo sysctl -w net.ipv4.conf.wg0.forwarding=1` turns it on until the next reboot.
