...
| 9090/tcp | Cockpit (HTTPS) | LAN, WireGuard | Set with `router.cockpit.port` in the host flake. Over WireGuard, connect to the LAN gateway address or `<hostName>.<lan.domain>`: Cockpit refuses logins at the router's tunnel address. See [The web UI](/docs/start/cockpit/#sign-in). |
...
| --- | --- | --- | --- | --- |
| 80/tcp, 443/tcp | Block page (Technitium Block Page app) | `accessPolicies.blockPage.enable` | LAN, WireGuard, guest | Bound to every address, with a self-signed certificate. Blocked names resolve to the LAN gateway address, where the page answers. With the reverse proxy on, see the hairpin note under [Redirected and blocked traffic](#redirected-and-blocked-traffic). |
