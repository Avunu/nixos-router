...
- **Names moved to or from dynamic DNS:** [dynamic DNS](/docs/dynamic-dns/) tags its records with the same comment, and a name can move between the two in a single **Save & apply**, with the two services running in no fixed order. The tunnel deletes a dynamic DNS record at a name it takes over without remembering it, since it was never yours: dynamic DNS remembers what it replaced there and puts that back itself. When you move a name from the tunnel to dynamic DNS and dynamic DNS runs first, the tunnel finds dynamic DNS's A or AAAA record at the name. It then keeps the record it remembered rather than dropping it, and every later sync tries again, including syncs with the tunnel off. Once the name leaves dynamic DNS too, the next sync puts the record back. A record tagged `managed by nixos-router` never counts as one made by hand under the rule above.
