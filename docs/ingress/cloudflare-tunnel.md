...
- **DNS records:** one row per hostname, **ok** or **error**, with what the last sync did, such as `created`, `unchanged`, or a note that an existing record was replaced. A name you removed also gets a row while the record it replaced is waiting to be put back, with "waiting until dynamic DNS releases the name" (see below).
...
- **Names moved to or from dynamic DNS:** [dynamic DNS](/docs/dynamic-dns/) tags its records with the same comment, and a name can move between the two in a single **Save & apply**, with the two services running in no fixed order. The tunnel deletes a dynamic DNS record at a name it takes over without remembering it, since it was never yours: dynamic DNS remembers a CNAME it replaced there and puts that back itself. When you move a name from the tunnel to dynamic DNS and dynamic DNS runs first, the tunnel finds dynamic DNS's A or AAAA record at the name. It then keeps the record it remembered rather than dropping it, and every later sync tries again, including syncs with the tunnel off. Once the name leaves dynamic DNS too, the next sync puts the record back. When the tunnel runs first, it puts the record back at once and dynamic DNS takes the name over from there. A CNAME is then remembered by dynamic DNS and comes back when the name leaves it. An A or AAAA record of a type dynamic DNS publishes is overwritten, because dynamic DNS doesn't remember those, so it's lost. A record tagged `managed by nixos-router` never counts as one made by hand under the rule above, and one that state from an older version remembered is never put back.
...
2. Click **Save & apply**.
3. The apply runs a sync, and **Sync now** runs another. It deletes the tunnel's CNAMEs and puts back any records they replaced, deletes the tunnel, and removes the credentials file. When the **Tunnel status** card shows **Last sync** **ok** and **Tunnel** **none**, it's done, unless a row under **DNS records** says "waiting until dynamic DNS releases the name". Dynamic DNS still holds that name, and a later sync puts the record back once it lets go.
