...
# tabs write to (via nix-editor) when you change options in the web UI. Keeping
# every `router.*` value in one flat module (rather than inline in flake.nix)
# is what lets the UI address option paths like `router.dns.adguard.safeSearch`.
#
# It is a normal NixOS module: you can still hand-edit anything here. The web UI
# only ever touches the specific options exposed by its forms (filtering, DNS,
# firewall/port-forwards, UPnP, IPS, DHCP) — network topology stays under your
# control here.
{
  router = {
    hostName = "948-router";
    ...
    # ── UPnP-IGD / NAT-PMP (disabled by default) ─────
    upnp.enable = false;
